...
public class Blocks : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (time >= timeDelay)
        {
            ...
            RadNum = Random.Range(1,8);
            Vector2 randPos = new Vector2(Random.Range(-6,8),80);
            if (RadNum == 1)
            { 
            ...
            {
                Instantiate(triangle,randPos,Quaternion.identity);
            }
            else
            {
                Instantiate(hexagon,randPos,Quaternion.identity);
            }
        }
    }
}
